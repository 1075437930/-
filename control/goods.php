 ...
 * ============================================================================
 * $Author: 整理 萤火虫 $
 * $Id: goods.php  2018年5月28日09:08:49   萤火虫 $
 */
...
class goodsControl extends BaseControl{
    ...
    public function lists(){
        ...
        Tpl::assign('ur_here',      L('01_goods_list'));
        Tpl::assign('action_link',  array('text' => L('03_goods_add'), 'href' => 'index.php?act=goods&op=goods_add'));
    ...
    public function goods_query() {
        ...
        $sort_flag  = sort_flag($goods_list['filter']);
        Tpl::assign($sort_flag['tag'],$sort_flag ['img']);
        make_json_result(Tpl::fetch('goods_list.htm'), '',
     ...
     */
     public function goods_add() {
        ...
        $this->GoodsPage($goods);
        Tpl::assign('goods', $goods);
        Tpl::assign('ur_here',     L('03_goods_add'));
        Tpl::assign('action_link', array('text' => L('01_goods_list'), 'href' => 'index.php?act=goods&op=lists'));
        Tpl::assign('form_act', 'goods');
        Tpl::assign('form_op', 'goods_insert');
        Tpl::display('goods_info.htm');
     }
    ...
    */
    public function goods_insert() {
        ...
        if ((isset($_FILES['goods_img']['error']) && $_FILES['goods_img']['error'] == 0) || (!isset($_FILES['goods_img']['error']) && isset($_FILES['goods_img']['tmp_name']) && $_FILES['goods_img']['tmp_name'] != 'none')){
            ...
            $numsd = rand(0,99);
            // 格式化图片地址直接存入oss
            $res = reformat_image_name($_FILES['goods_img'],'zheng',$numsd);
            if (!empty($res)){
                $img_url = $res;
            }else{
        ...
        /* 保存商品相册图片 */
        $this->GoodsImgSave($goodsids);
        /* 记录日志 */
        admin_log($insert['goods_name'].'('.$goods_sns.') '.'分销价格：'.$insert['goods_fenxiao_price'], 'add', 'goods');
        $link = array();
        $link['text'] = L('back_lists');
        $link['href'] = 'index.php?act=goods&op=lists';
        admin_log($article_title,'add','article');
        showMessage(L('add_goods_ok'),$link);
    }
    ...
    */
    public function goods_edit() {
        ...
        Tpl::assign('action_link', array('text' => L('01_goods_list'), 'href' => 'index.php?act=goods&op=lists'));
        Tpl::assign('form_act', 'goods');
        Tpl::assign('form_op', 'goods_update');
        Tpl::display('goods_info.htm');
    }
    ...
    */
    public function goods_remove() {
        ...
        if ($resule){
            ...
            admin_log(addslashes($goods_row['goods_name']), 'trash', 'goods'); // 记录日志
            $url = 'index.php?act=goods&op=goods_query';
            ecs_header("Location: $url\n");
            exit;
        }
    ...
    */
    public function edit_goods_number(){
        ...
        if(!empty($goods_info)){
            ...
            if (!empty($result)){
                 //添加修改产品编号日志
                admin_log($goods_info['goods_name'].'('.$goods_info['goods_sn'].')'.' 库存从'.$goods_info['goods_number'].'改为：'.$goods_num, 'edit', 'goods');
                clear_cache_files();
    ...
    public function edit_goods_sn(){
        ...
        if (!empty($result)){
            //添加修改产品编号日志
            admin_log($goods_info['goods_name'].'('.$goods_sn.')'.' 原产品编号：'.$goods_info['goods_sn'], 'edit', 'goods');
            clear_cache_files();
    ...
    public function goods_batch(){
        ...
        if (isset($_POST['type'])){
            ...
            if ($_POST['type'] == 'trash'){
                ...
                if(!empty($sales_pro_list)){
                    $link[] = array('href' => 'index.php?act=goods&op=lists', 'text' => L('01_goods_list'));
    ...
    private function GoodsList(){
        ...
        if ($result === false){   
            ...
            $filter['sort_order']       = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
             /* 默认实物产品 */
            $where = ' is_real = 1 AND is_chou = 0 AND is_delete = 0 AND supplier_id = 0 ';
            /* 品牌 */
            ...
            if (!empty($filter['cat_id'])) {
                $where .= " AND (cat_ids_new LIKE '%,".$filter['cat_id']."%' OR cat_ids_new LIKE '%".$filter['cat_id'].",%')";
            }
            /* 关键字 */
            ...
            $sql = "SELECT *  FROM " . Model()->tablename('goods') . " WHERE  $where" .
     ...
     */
    private function GoodsImgSave($goods_id,$is_insert){
    ...
    public function ces(){
        ...
        foreach($arr as $k=>$v) {
            //修改图片路径
            $arr[$k]['jb'] = get_imgurl_oss($v['original_img'],40,40,false,true);

            $arr[$k]['jb']="<img src='{$arr[$k][jb]}'>";
            $arr[$k]['add_time'] = local_date('Y-m-d',$v['add_time']);
            $arr[$k]['last_update'] = local_date('Y-m-d',$v['last_update']);
        }
    ...
    */
     ...
     * 
     */
    private function GoodsPage($goods){
    ...
    /*
       @return 删除商品到回收站
    */
    public function trash_remove(){

        $edit=[];

        $edit['is_delete']=1;

        $info=Model('goods')->select_goods_info("*","goods_id={$_GET['id']}");
        if(Model('goods')->update_goods($edit,"goods_id={$_GET['id']}")){
            /* 记录日志 */
            admin_log($info['goods_name'], 'trash', 'goods');
    ...
    */
    public function trash(){
        ...
        Tpl::assign('ur_here', L('11_goods_trash'));
        $action_link = ['text' => L('01_goods_list'), 'href' => 'index.php?act=goods&op=lists'];


        Tpl::assign('action_link', $action_link);

        Tpl::display('trash_list.htm');
    ...
    public function huanyuan(){
        ...
        if(Model('goods')->update_goods($edit,"goods_id={$_GET['id']}")){
            /* 记录日志 */
            admin_log($info['goods_name'], 'restore', 'goods');

            showMessage(L('hy_succ'), ['text' =>L('hy_succ'), "href" => "index.php?act=goods&op=trash"]);
        }
    ...
    public function trash_del(){
        $info=Model('goods')->select_goods_info("*","goods_id={$_GET['id']}");
        if(Model('goods')->delete_goods("goods_id={$_GET['id']}")){
            /* 记录日志 */
            admin_log($info['goods_name'], 'remove', 'goods');
    ...
    public function GetTypeList(){
        ...
        if ($result === false){
            ...
            $goods_type = Model('goods');
            /*搜索相关start*/
            $filter['cat_name'] = !empty($_POST['cat_name']) ? $_POST['cat_name']:'';
            $where_brand = " 1 AND is_delete=1";
            if($filter['cat_name']){
                $where_brand .= " AND goods_name like '%".$_POST['cat_name']."%'";
            }
            /*搜索相关end*/

            $filter['record_count'] =$goods_type->get_goods_count($where_brand);

            $filter = page_and_size($filter);
            /* 查询记录 */
            $sql = "SELECT * FROM ".Model()->tablename('goods')." WHERE ".$where_brand." ORDER BY  goods_id DESC ";
            set_filter($filter, $sql);
        }else{
            $sql    = $result['sql'];
            $filter = $result['filter'];
    ...
    public function type_query() {
        ...
        Tpl::assign($sort_flag['tag'], $sort_flag['img']);
        make_json_result(Tpl::fetch('trash_list.htm'), '',array('filter' => $brand_list['filter'], 'page_count' => $brand_list['page_count']));
    }

    public function chuli(){
        $goods=Model('goods')->get_goods_list('*','');
        foreach($goods as $val){
            $add=[];
            $add['item_id']=$val['goods_id'];
            $add['type']='Goods';
            $add['goods_name']=$val['goods_name'];
            $add['goods_sn']=$val['goods_sn'];
            $add['starttime']='0000-00-00';
            $add['endtime']='0000-00-00';

            Model('automanage')->insert_auto_manage($add);
        }
    }
}
