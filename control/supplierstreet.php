...
class supplierstreetControl extends BaseControl {
    ...
    public function edit_info() {
        ...
        $info = Model('street_category')->select_supplier_street_info('*', $where);
    ...
    public function saveinfo() {
        ...
        $where['supplier_id'] = $suppid;
        $result = Model('street_category')->update_supplier_street($save, $where);
        if ($result) {
    ...
    public function remove_show() {
        ...
        Model('street_category')->update_supplier_street($update, $where);
        /* 返回 */
        $link[] = array('text' => L('back_to_supplier_street_list'), 'href' => 'index.php?act=supplierstreet&op=lists');
        showMessage(L('attradd_succed'), $link);
    }
    ...
    public function remove_supplier() {
        ...
        Model('street_category')->delete_supplier_street($where);
        /* 返回 */
        $link[] = array('text' => L('back_to_supplier_street_list'), 'href' => 'index.php?act=supplierstreet&op=lists');
        showMessage(L('attradd_succed'), $link);
    }
    ...
    public function edit_sort_order() {
        ...
        $result = Model('street_category')->update_supplier_street($up_arr, $where);
        if ($result) {
    ...
    public function toggle_status() {
        ...
        $where['supplier_id'] = $id;
        $result = Model('street_category')->update_supplier_street($info, $where);
        if ($result) {
            make_json_result($val);
        } else {
            make_json_result('','database error',array('error'=>1));
     ...
     * @return array
     */
    private function get_street_list() {
        $result = get_filter();
        if ($result === false) {
            ...
            if ($_REQUEST['is_show'] !== false) {
                $where .= " AND supplier_street.is_show=" . $_REQUEST['is_show'];
            }
            ...
            $filter['record_count'] = Model('street_category')->get_supplier_street_count($where);
            /* 分页大小 */
            $filter = page_and_size($filter);
            $sql = "SELECT supplier_street.*,street_category.str_name " .
                    " FROM " . Model()->tablename('supplier_street') . " AS supplier_street " .
                    " LEFT JOIN" . Model()->tablename('street_category') . " AS street_category " .
                    " ON supplier_street.supplier_type = street_category.str_id " .
                    " $where" .
     ...
     * @param
     */
    private function get_street_type() {
        $info = Model('street_category')->get_street_category_list('*', 'is_show = 1');
        $ret = array();
        foreach ($info as $k => $v) {
     ...
     */
    private function get_tag_map($suppid) {
        $supplier_model = Model('supplier');
        $ret = array();
