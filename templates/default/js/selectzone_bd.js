...
function SelectZone()
{
   ...
   */
  this.loadOptions = function(act,op,keys,filters)
  {
    Ajax.call(this.filename+"&act=" + act+"&op=" + op+keys, filters, this.loadOptionsResponse, "GET", "JSON");
  }
   ...
   */
  this.addItem = function(all, act,op,arguments)
  {
    if (!this.check())
    {
      return;
    ...
    if (selOpt.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&op=" + op+"&add_ids=" + selOpt, args, this.addRemoveItemResponse, "GET", "JSON"); /* mod by wengwenjin 百度编辑器 */
    }
  }
   ...
   * @param   string     act
   */
  this.dropItem = function(all, act,op)
  {
    if (!this.check())
    {
      return;
    ...
    if (arr.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&op=" + op+"&drop_ids=" + arr, args, this.addRemoveItemResponse, 'GET', 'JSON'); /* mod by wengwenjin 百度编辑器 */
    }
  }
