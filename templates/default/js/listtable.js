...
listTable.edit = function(obj, act, id)
{
  ...
  txt.onblur = function(e)
  {
    ...
    {
      res = Ajax.call(listTable.url, "act="+listTable.act+'&op='+act+"&val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id, null, "POST", "JSON", false);
...
listTable.toggle = function(obj, act, id)
{
  ...
  var res = Ajax.call(this.url, "act="+listTable.act+'&op='+act+"&val=" + val + "&id=" +id, null, "POST", "JSON", false);
  ...
  if (res.error == 0)
  {
    obj.src = (res.content > 0) ? 'templates/default/images/yes.gif' : 'templates/default/images/no.gif';
  }
  
  if(res.append)
  { 
	 if(res.content > 0){
	   var pn = obj.parentNode.previousSibling.previousElementSibling;
	   pn.textContent = res.append;
	 }
  }
}
...
{
  var args = "act="+listTable.act+'&op='+this.query+"&sort_by="+sort_by+"&sort_order=";
...
listTable.loadList = function()
{
  ...
  var args = "act="+listTable.act+'&op='+this.query + this.compileFilter();
...
listTable.remove = function(id, cfm, opt)
{
  ...
  if (confirm(cfm))
  {
    var args = "act="+listTable.act+'&op=' + opt + "&id=" + id + this.compileFilter();
 ...
 * 更改审核状态
 */
listTable.state = function(id, cfm, opt)
{
  if (opt == null)
  {
    opt = "refuse";
...
/*加载更多start*/
listTable.getmore = function()
{
    if (this.filter.page < listTable.pageCount)
    {
        listTable.gotoPage2(parseInt(this.filter.page) + 1);
    }
}
 ...
 * 翻页
 */
listTable.gotoPage2 = function(page)
{
    if (page != null) this.filter['page'] = page;
 ...
 * 载入列表
 */
listTable.loadList2 = function()
{

    var args = "act="+listTable.act+'&op='+this.query + this.compileFilter();

    Ajax.call(this.url, args, this.listCallback2, "POST", "JSON");
}


listTable.listCallback2 = function(result, txt)
{
    if (result.error > 0)
    {
        alert(result.message);
    ...
    else
    {
        try
        {
            document.getElementById('listDiv').innerHTML = document.getElementById('listDiv').innerHTML+result.content;
...
listTable.getPageSize = function()
{
  ...
  if (pageSize)
  {
    ps = Utils.isInt(pageSize.value) ? pageSize.value : 30;
    document.cookie = "page_size=" + ps + ";";
  }
}
